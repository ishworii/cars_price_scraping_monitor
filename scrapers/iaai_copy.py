...
def extract_details_from_page(driver):
    logger.info("Extracting details from the current page...")
    data = []
    ...
    for each_div in divs:
        ...
        data.append(tmp)
    logger.info(f"Extracted {len(data)} items from the current page.")
    return data
...
def click_on_next(driver):
    logger.info("Trying to click on the next button...")
...
def extract_all_data(url, headless=False, proxy_server=None):
    ...
    while current_count <= total_pages:
        logger.info(f"Processing page {current_count} of {total_pages}.")
        ...
        sleep(3)
    driver.quit()
    logger.info(f"Data extraction finished. Extracted data for {len(res)} vehicles.")
    return res
